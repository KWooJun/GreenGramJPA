...
public class WithAuthMockUserSecurityContextFactory implements WithSecurityContextFactory <WithAuthUser>{
    ...
    @Override
    public SecurityContext createSecurityContext(WithAuthUser annotation) { //annotation 으로 WithAuthUser 정보가 주입된다
        ...
        Authentication auth = new UsernamePasswordAuthenticationToken(myUserDetails
                                    , null, myUserDetails.getAuthorities());
        securityContext.setAuthentication(auth); // 이 작업이 필요했음 , 시큐리티 인증이 되었다고 처리가 된다

        return securityContext; //테스트 떄 사용하는 인증처리가 될 것이다.
    }
}
